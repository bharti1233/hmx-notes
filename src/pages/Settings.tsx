  ...
  return (
    <>
          ...
          {AI_PROVIDERS.map(p => (
            <button key={p.id} onClick={() => update('provider', p.id)}
              className={`flex items-center gap-2.5 p-3 rounded-xl border transition-all text-left ${
                settings.provider === p.id ? 'border-primary bg-primary/10 shadow-soft' : 'border-border/40 bg-card hover:bg-muted/50'
              }`}>
      ...
      <Section title="API Keys">
        <div className="rounded-2xl bg-card border border-border/40 shadow-soft divide-y divide-border/30 overflow-hidden">
          <ApiKeyField label="Gemini API Key" value={settings.geminiApiKey} onChange={v => update('geminiApiKey', v)}
            show={!!showKeys.gemini} onToggleShow={() => toggleShow('gemini')} active={settings.provider === 'gemini'} />
          <ApiKeyField label="Groq API Key" value={settings.groqApiKey} onChange={v => update('groqApiKey', v)}
            show={!!showKeys.groq} onToggleShow={() => toggleShow('groq')} active={settings.provider === 'groq'} />
          <ApiKeyField label="Claude API Key" value={settings.claudeApiKey} onChange={v => update('claudeApiKey', v)}
            show={!!showKeys.claude} onToggleShow={() => toggleShow('claude')} active={settings.provider === 'claude'} />
        </div>
      </Section>
        ...
        <Section title="Custom AI Endpoint">
          <div className="rounded-2xl bg-card border border-border/40 p-4 shadow-soft space-y-3">
            <div>
              ...
              <input type="url" placeholder="https://api.your-provider.com/v1/chat/completions"
                value={settings.customUrl} onChange={e => update('customUrl', e.target.value)}
            ...
            <div>
              ...
              <div className="relative">
                <input type={showKeys.custom ? 'text' : 'password'} placeholder="sk-..."
                  value={settings.customApiKey} onChange={e => update('customApiKey', e.target.value)}
      ...
      <Section title="System Prompt">
        <div className="rounded-2xl bg-card border border-border/40 p-4 shadow-soft space-y-3">
          ...
          <textarea value={settings.systemPrompt} onChange={e => update('systemPrompt', e.target.value)} rows={5}
          ...
          <button onClick={() => { reset(); toast.success('AI settings reset to defaults'); }}
...
function ApiKeyField({ label, value, onChange, show, onToggleShow, active }: {
  ...
  return (
    <div className={`p-4 space-y-1.5 ${active ? 'bg-primary/5' : ''}`}>
      ...
      <div className="relative">
        <input type={show ? 'text' : 'password'} placeholder="Enter API key..."
          value={value} onChange={e => onChange(e.target.value)}
          className="w-full h-9 px-3 pr-9 rounded-lg bg-muted/50 border border-border/40 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
        <button onClick={onToggleShow} className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 text-muted-foreground hover:text-foreground">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      <p className="text-xs font-bold uppercase text-muted-foreground tracking-wider mb-2 px-1">{title}</p>
      {children}
    </div>
  );
}
...
function Row({ icon: Icon, title, description, trailing }: {
  icon: typeof Moon; title: string; description: string; trailing?: React.ReactNode;
}) {
  return (
    <div className="rounded-2xl bg-card border border-border/40 p-4 shadow-soft flex items-center gap-3">
      <div className="w-9 h-9 rounded-xl bg-primary-soft flex items-center justify-center shrink-0">
...
function useLocalBool(key: string, defaultValue: boolean): [boolean, (v: boolean) => void] {
  ...
  const update = (v: boolean) => { setVal(v); localStorage.setItem(key, String(v)); };
  return [val, update];
}
