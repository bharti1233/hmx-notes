...
import { StickyNote, LayoutGrid, Settings as SettingsIcon } from 'lucide-react';
...
export function BottomNav() {
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 bg-card/95 backdrop-blur-md border-t border-border">
      <div className="max-w-2xl mx-auto grid grid-cols-3 h-16 px-2 pb-[env(safe-area-inset-bottom)]">
            ...
            {({ isActive }) => (
              <>
                <div className={`flex items-center justify-center transition-all ${
                  isActive ? 'w-12 h-7 rounded-full bg-primary-soft' : ''
                }`}>
